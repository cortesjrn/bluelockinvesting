...
//LEAVE ABOVE FUNCTION TO FALSE RIGHT UNTIL AFTER LEAKS DROP (SATURDAY/SUNDAY) THEN SET IT TO TRUE AFTER STOCKS ARE UPDATED (AFTER SUNDAY, SET BACK TO FALSE)
...
app.post("/buy-stock",authenticateToken, (req, res) => {
  ...
  let flag = false;
  const date = new Date();
  //HERE IS WHERE YOU CHANGE THE DATE OF INSIDER TRADING
  if((date.getDay()==7 || (date.getDay()==6&&date.getHours()==15))&&!enableInsiderTrading){
    res.json("Buying and selling stocks is closed between Saturday (when leaks drop) and when I update the stock values (asap) to prevent people from basicially insider trading, sorry about any inconvinience this may cause");
  }
  else{
  db.select(
    "kaiser",
...
app.post("/sell-stock",authenticateToken, (req, res) => {
  ...
  const date = new Date();
  //HERE IS WHERE YOU CHANGE THE DATE OF INSIDER TRADING
  if((date.getDay()==7 || (date.getDay()==6&&date.getHours()>=10))&&!enableInsiderTrading){
    res.json("Buying and selling stocks is closed between Saturday (when leaks drop) and when I update the stock values (asap) to prevent people from basicially insider trading. Sorry about any inconvinience this may cause.");
  }
  else{
  db.select(
